...
from word_pool import PoolableBertEmbeddings
...
class CascadeXML_multicls(nn.Module):
    def __init__(self, params, train_ds):
        super(CascadeXML_multicls, self).__init__()
        ...
        self.attn_drops = nn.ModuleList([nn.Dropout(0.1) for p in range(len(embed_drops))])
        ...
        self.Cn_word = nn.ModuleList([nn.Embedding(out_s, embed_size) for i, out_s in enumerate(self.num_ele[:-1])])
        self.Cn_word.append(nn.Embedding(self.num_ele[-1]+1, embed_size, padding_idx=-1))
    ...
    def init_classifier_weights(self):
        ...
        # for pw_emb in self.pw_attention:
        #     nn.init.xavier_uniform_(pw_emb.weight)
        
        # for bias in self.Cn_word_bias:
        #     bias.weight.data.fill_(0)
        
        for C in self.Cn_word:
            nn.init.xavier_uniform_(C.weight)
        self.Cn_word[-1].weight[-1].data.fill_(0)


    def feat_word_maker(self, hidden_states, recipe):
        outs = []
        for level, layers in enumerate(recipe):
            if isinstance(layers, tuple):
                layer = layers[-1]
            else:
                layer = layers
            word_hidden_states = hidden_states[layer][:, 1:self.seq_len - 1, :]
            outs.append(word_hidden_states)
    ...
    def forward(self, input_ids, attention_mask, epoch, all_labels = None, use_precomputed=False, use_r=False, return_out=False):
        ...
        if not use_precomputed:
            ...
            out_words = self.feat_word_maker(bert_outs, self.layers)
        ...
        for i, (embed, embed_word, feat, feat_word) in enumerate(zip(self.Cn, self.Cn_word, outs, out_words)):
            if i == 0:
            ...
            attn_wts = embed_word(candidates)
            attn_map = torch.bmm(feat_word, attn_wts.permute(0,2,1))
            attn_map = self.attn_drops[i](F.softmax(attn_map, dim = 1))
            word_rep = torch.bmm(attn_map, attn_wts)
            word_rep = torch.sum(word_rep, dim = 1)
            feat = feat + word_rep.unsqueeze(-1)
            ...
            # attention_wts = torch.matmul(word_hidden_states, self.pw_attention[level].sigmoid())
            # attention_wts = F.softmax(attention_wts, dim = 2)
            # feat_word = torch.sum(attention_wts*word_hidden_states, dim = 1).unsqueeze(-1)
