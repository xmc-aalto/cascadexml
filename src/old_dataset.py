...
def createDataCSV(dataset):
    ...
    print(f'Labels in training data = {len(label_map)}')
    ...
    with open(f'./data/{dataset}/test_labels.txt') as f:
        for i in tqdm.tqdm(f):
            for l in i.replace('\n', '').split():
                label_map[l] = 0
            labels.append(i.replace('\n', ''))
    print(f'Labels after adding test data = {len(label_map)}')
...
class MDataset(Dataset):
    def __init__(self, df, mode, tokenizer, label_map, max_length,
        ...
        if group_y is not None:
            ...
            self.map_group_y = np.zeros(len(label_map), dtype=np.long)-1
    ...
    def __getitem__(self, idx):
        ...
        if self.group_y is not None:
            ...
            group_labels = np.array([g for g in group_labels if g != -1], dtype=np.int64)
            ...
            if self.mode == 'train':
                # candidates = np.concatenate([np.array(labels), np.zeros((len(candidates) - len(labels))) - 1])
                return input_ids, attention_mask, token_type_ids,\
                    label_ids[candidates], group_label_ids, candidates
                    # labels, group_label_ids, candidates
            else:
                return input_ids, attention_mask, token_type_ids,\
